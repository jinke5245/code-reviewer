/** Side of a GitLab diff finding anchor. */
export type GitLabDiffSide = "new" | "old";

/** Parsed line from a GitLab unified diff hunk. */
...
export type GitLabDiffRange = {
  endLine: number;
  startLine: number;
};

/** Parses GitLab unified diff text into side-aware line records. */
export function parseGitLabDiffLines(diff: string): GitLabDiffLine[] {
  const lines: GitLabDiffLine[] = [];
...
export function readGitLabDiffRangeCode({
  diff,
  endLine,
  side,
  startLine,
  ...
  const parsedLines = parseGitLabDiffLines(diff);
...
export function findGitLabDiffRangeForCode({
  code,
  diff,
  preferredStartLine,
  side,
  ...
  side: GitLabDiffSide;
}): GitLabDiffRange | undefined {
  const codeLines = code.split(/\r?\n/u);
  const sideLines = parseGitLabDiffLines(diff).flatMap((line) => {
